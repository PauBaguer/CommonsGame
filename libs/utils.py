...
def plot_learning_curve(x, scores, epsilons, loss, filename):
    fig = plt.figure(1)
    fig.clf()
    ...
    for i, m in enumerate([loss, scores]):
        df = pd.DataFrame(m, columns=['value'])
        df['rolling_avg'] = df['value'].rolling(window=window_size, min_periods=25).mean()

        axs[i].plot(df.index, df['value'], alpha=0.5, color=colors[i])
        axs[i].plot(df.index, df['rolling_avg'], color=colors[i])
